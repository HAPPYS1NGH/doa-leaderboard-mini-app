...
export default function ClaimPage() {
  ...
  // Derive authentication state from Farcaster context and wallet connection
  const authenticated = isSDKLoaded && context?.user?.fid && isConnected;
    ...
    try {
      ...
      for (const address of authorizedAddresses) {
        try {
          ...
          if (response.ok && result.success) {
            const addressResult = result.data.find((item: { address: string; ensName?: string }) => 
  ...
  // Fetch Farcaster username from Neynar API
  const fetchUsernameFromAPI = async (fid: number) => {
    ...
    try {
      ...
      if (response.ok && result.fetchedUsername) {
        ...
        setFetchedUsername(context?.user?.username || "");
      }
    } catch (error) {
  ...
  const createSubname = async () => {
    ...
    if (!context?.user?.username) {
      setError("Farcaster username not available");
      return;
    }
    ...
    try {
      const response = await fetch("/api/subname/create", {
        ...
        body: JSON.stringify({
          ...
          // Send all Farcaster context data
          farcasterData: {
            fid: context.user.fid,
            username: context.user.username,
            displayName: context.user.displayName,
      ...
      if (response.ok) {
        const finalUsername = result.fetchedUsername || context.user.username;
        setSubnameCreated(`${finalUsername}.deptofagri.eth`);
  ...
  // Fetch Farcaster username when user is authenticated
  useEffect(() => {
    if (authenticated && context?.user?.fid) {
      fetchUsernameFromAPI(context.user.fid);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [authenticated, context?.user?.fid]);
    ...
    <>
      ...
      <div className="min-h-screen bg-cream">
        <div className="max-w-4xl mx-auto px-4 py-8 md:py-12">
          ...
          <div className="bg-white/90 backdrop-blur-sm rounded-2xl shadow-xl border border-forest/20 p-6 md:p-8 max-w-2xl mx-auto">
            {!authenticated ? (
              ...
              <div className="space-y-6">
                <div className="text-center">
                  ...
                  <div className="mt-4">
                    ...
                    >
                      {showManualInput ? "Use wallet" : "Check with Hat address"}
                    </button>
                  </div>
                </div>
                  ...
                  <div className="space-y-3">
                    <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
                      ...
                      <p className="text-xs md:text-sm text-blue-700 mb-3">
                        Enter hat address to check authority. Send USDC to the connected Farcaster address to claim subname.
                      </p>
                  ...
                  !isValidatingEligibility && (
                    <div className="bg-orange-50 border border-orange-200 rounded-xl p-4">
                      ...
                      <p className="text-xs md:text-sm text-orange-700">
                        No authority to claim subnames. Only for addresses your Hat sent your USDC to.
                      </p>
                    </div>
                  )}
                    ...
                    <div className="space-y-2">
                      ...
                      >
                        {eligibleAddresses.map((address) => {
                          ...
                          const label = `${address.slice(0, 6)}...${address.slice(
                            -4
                          )}`;
                          let relationship = "";
                    ...
                    {existingSubnames.length === 0 && (
                      <div className="space-y-2">
                        ...
                        <div className="relative">
                          <div className="w-full p-3 border border-forest/20 rounded-lg bg-forest/5 text-forest font-mono text-sm pr-32">
                            {context?.user?.username || "Loading..."}
                          </div>
                        ...
                        <div className="text-xs text-forest/60 mt-2">
                          {isFetchingUsername ? (
                            <div className="flex items-center space-x-2">
                              <div className="animate-spin rounded-full h-3 w-3 border-b-2 border-forest"></div>
                              <span>Fetching official Farcaster username...</span>
                            </div>
                          ) : fetchedUsername && fetchedUsername !== context?.user?.username ? (
                            <div className="space-y-1">
                              <div>Using official Farcaster username: <strong className="text-forest">{fetchedUsername}</strong></div>
                              <div className="text-forest/50">(instead of: {context?.user?.username})</div>
                            </div>
                          ) : (
                            <div>Using your Farcaster username: <strong>{context?.user?.username}</strong></div>
                          )}
                        </div>
          ...
          <div className="mt-8 text-center">
            <p className="text-xs text-forest/50">
              By connecting wallet and claiming subname, you agree to be a good farmer and tap your hat every friday.
            </p>
          </div>
        </div>
      </div>
